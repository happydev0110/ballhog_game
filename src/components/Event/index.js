...
import { URL, SPORTS_CATEGORY, INTERVAL_TIME, DATASET_TYPE_CATEGORY, Logo } from '../../const.js';
...
function EventComponent(props) {
    const { primaryColor, secondaryColor, tabStatus, setPrimaryColor, setSecondaryColor, handleTab } = props;
    ...
    const [sportCategory, setSportCategory] = useState('NFL');
    ...
    // const [showTeamOption, setShowTeamOption] = useState(false);
    ...
    return (
        <>
                ...
                tabStatus && <>
                    ...
                    <div className='row my-3'>
                        ...
                        <div className='col-md-12 mt-3'>
                            ...
                            <select className="form-select form-select-sm"
                                ...
                                onChange={evt => {
                                    ...
                                    setTeam1Idx(team1)
                                    var team2 = (parseInt(team1) + 1) % 2;
                                    // set Primary and Secondary Color
                                    var primary_col = playList.boxscore.teams[team1].team.color;
                                    var secondary_col = playList.boxscore.teams[team1].team.alternateColor;
                                    if (!primary_col.includes('#')) {
                                        setPrimaryColor('#' + primary_col)
                                    }

                                    if (!secondary_col.includes('#')) {
                                        setSecondaryColor('#' + secondary_col)
                                    }

                                    setTeam2Name(team1 != -1 && playList.boxscore.teams[(parseInt(team1) + 1) % 2].team.name)
                                    setSelectedTeam1s([team1, team2, team2, team2])
                                }}
                            >
                        ...
                        <div className='text-center mt-3 mb-2'>
                            <button className='btn btn-primary' onClick={handleTab} style={{ backgroundColor: primaryColor, color: secondaryColor }}>Go To Game</button>
                        </div>
                        <div className='col-md-12'>
                ...
                !tabStatus && <>
                    ...
                    <div className='d-flex justify-content-between p-4'>
                        <div className='d-inline-block'>
                            <img src={team1Idx != -1 ? playList.boxscore.teams[team1Idx].team.logo : Logo[sportCategory]} style={{ width: 60, height: 60 }} />
                        </div>
                        <div className='d-inline-flex align-items-center'>
                            <div className='d-inline-block'>
                                <img src={team1Idx != -1 ? playList.boxscore.teams[team1Idx].team.logo : Logo[sportCategory]} style={{ width: 40, height: 40 }} />
                                <p className='px-2 d-inline-block'><b>{(team1Idx != -1 && team1Idx == 0) ? awayScore : homeScore}</b></p>
                            </div>
                            <div className='d-inline-block'>
                                <img src={team1Idx != -1 ? playList.boxscore.teams[(parseInt(team1Idx) + 1) % 2].team.logo : Logo[sportCategory]} style={{ width: 40, height: 40 }} />
            ...
            />
            {/* <CustomModal
                modalIsOpen={showTeamOption}
                primaryColor={primaryColor}
                setModalIsOpen={setShowTeamOption}
                onRequestClose={() => setShowTeamOption(false)}
            >
                <div className='col-md-12 mt-3'>
                    <label className="form-label" style={{ float: "left" }}>Team Your Cheering For</label>
                    <select className="form-select form-select-sm"
